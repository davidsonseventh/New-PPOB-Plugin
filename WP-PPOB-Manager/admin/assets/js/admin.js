(function ($) {
    ...
    // --- FUNGSI TAMBAH PRODUK VIA SKU DI KATEGORI ---
    const skuInputBtn = $('#wppob-add-products-by-sku');
    if (skuInputBtn.length) {
        skuInputBtn.on('click', function(e) {
            e.preventDefault();

            const skuTextarea = $('#wppob-sku-input');
            const feedbackEl = $('#wppob-sku-feedback');
            const productCheckboxContainer = $('.wppob-product-checkbox-container');
            const skusToFind = skuTextarea.val().split(/\s+/).filter(Boolean); // Memisahkan SKU berdasarkan spasi atau baris baru

            if (skusToFind.length === 0) {
                feedbackEl.text('Silakan masukkan satu atau lebih SKU.').css('color', 'red');
                return;
            }
            ...
            // Iterasi melalui setiap SKU yang dimasukkan
            skusToFind.forEach(function(sku) {
                const checkbox = productCheckboxContainer.find('input[data-sku="' + sku.trim() + '"]');
                if (checkbox.length) {
                    checkbox.prop('checked', true); // Centang kotak jika SKU ditemukan
                    foundCount++;
                } else {
                    notFoundSkus.push(sku.trim());
                }
            });
            
            // Scroll ke produk pertama yang ditemukan agar pengguna bisa melihat hasilnya
            if (foundCount > 0) {
                 const firstChecked = productCheckboxContainer.find('input[data-sku="' + skusToFind[0].trim() + '"]:checked');
                 if (firstChecked.length) {
                    productCheckboxContainer.scrollTop(0); // Reset scroll
                    productCheckboxContainer.scrollTop(
                        firstChecked.offset().top - productCheckboxContainer.offset().top + productCheckboxContainer.scrollTop() - 10
                    );
                }
            }
            
            // Berikan umpan balik kepada pengguna
            let feedbackMessage = `Berhasil menemukan dan mencentang ${foundCount} produk.`;
            if (notFoundSkus.length > 0) {
                feedbackMessage += ` SKU tidak ditemukan: ${notFoundSkus.join(', ')}.`;
                feedbackEl.css('color', '#d9534f');
            } else {
                feedbackEl.css('color', '#5cb85c');
            }
            feedbackEl.text(feedbackMessage);
            
            // Kosongkan textarea setelah selesai
            skuTextarea.val('');
        });
    }
