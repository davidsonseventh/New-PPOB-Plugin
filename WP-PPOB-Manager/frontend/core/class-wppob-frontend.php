...
add_action('wp_ajax_wppob_check_postpaid_bill', [$this, 'ajax_check_postpaid_bill']);
add_action('wp_ajax_wppob_process_topup', [$this, 'ajax_process_topup']);
// ... (sisa kode)
...
public function ajax_submit_purchase() {
    ...
    $profit = wppob_get_profit_for_user($product);
    $total_payment = wppob_get_price_for_user($product, $user_id);

    if (!WPPOB_Balances::deduct_balance($user_id, $total_payment, 'Pembelian ' . $product->get_name())) {
        wp_send_json_error(['message' => 'Saldo Anda tidak mencukupi.']);
    }

    $order_data = WPPOB_Orders::create_order([
        'user_id' => $user_id, 'product_code' => $product->get_sku(), 'customer_no' => $customer_no,
        'base_price' => $base_price, 'sale_price' => $total_payment, 'profit' => $profit, 'status' => 'processing'
    ]);
    $order_id = $order_data['id'];
    $transaction_ref_id = $order_data['ref_id'];

    $api_response = $api->create_transaction($product->get_sku(), $customer_no, $transaction_ref_id);

    if ($order_id > 0 && isset($api_response['data']['status'])) {
        WPPOB_Orders::update_order($order_id, ['api_response' => json_encode($api_response['data'])]);
        if (in_array(strtolower($api_response['data']['status']), ['gagal', 'error'])) {
            WPPOB_Orders::update_order($order_id, ['status' => 'failed']);
            if ($total_payment > 0) WPPOB_Balances::add_balance($user_id, $total_payment, 'Refund otomatis: Transaksi #' . $order_id . ' gagal.');
            wp_send_json_error(['message' => 'Transaksi Gagal: ' . ($api_response['data']['message'] ?? 'Alasan tidak diketahui.')]);
        } else {
            wp_send_json_success(['message' => 'Permintaan terkirim, menunggu konfirmasi.', 'transaction_id' => $order_id]);
        }
    } else {
        if ($order_id > 0) WPPOB_Orders::update_order($order_id, ['status' => 'failed', 'api_response' => 'Koneksi ke API Gagal.']);
        if ($total_payment > 0) WPPOB_Balances::add_balance($user_id, $total_payment, 'Refund otomatis: Gagal terhubung ke API.');
        ...
        if (isset($payload['ref_id'])) {
            ...
            if ($order && $order->status !== 'success' && $order->status !== 'failed') {
                ...
                if ($new_status === 'failed') {
                 ...
                 // ## TAMBAHKAN BARIS INI ##
    if (class_exists('WPPOB_Login_Core') && method_exists('WPPOB_Login_Core', 'send_transaction_notification')) {
        WPPOB_Login_Core::send_transaction_notification($order->id);
    }
            }
        }
...
/**
 * ## FUNGSI BARU ##
 * AJAX handler untuk memproses permintaan cek tagihan pascabayar.
 */
public function ajax_check_postpaid_bill() {
    check_ajax_referer('wppob_frontend_nonce', 'nonce');
    if (!is_user_logged_in()) {
        wp_send_json_error(['message' => 'Anda harus login untuk mengecek tagihan.']);
    }
    $product_id = isset($_POST['product_id']) ? intval($_POST['product_id']) : 0;
    $customer_no = isset($_POST['customer_no']) ? sanitize_text_field($_POST['customer_no']) : '';
    if (empty($product_id) || empty($customer_no)) {
        wp_send_json_error(['message' => 'Produk atau nomor pelanggan tidak boleh kosong.']);
    }
    $product = wc_get_product($product_id);
    if (!$product) {
        wp_send_json_error(['message' => 'Produk tidak ditemukan.']);
    ...
    $api = new WPPOB_API();
    $ref_id = 'INQ-' . uniqid();
    $response = $api->check_postpaid_bill($product->get_sku(), $customer_no, $ref_id);

    if (isset($response['data']['status']) && in_array($response['data']['status'], ['Sukses', 'Berhasil'])) {
        $bill_details = $response['data'];
        $admin_fee = (float) $product->get_meta('_wppob_base_price');
        $bill_amount = (float) $bill_details['price'];
        $profit_amount = wppob_get_profit_for_user($product);
        $total_payment = $bill_amount + $admin_fee + $profit_amount;

        $data_to_send = [
            'customer_name' => esc_html($bill_details['customer_name']),
            'period'        => esc_html($bill_details['period'] ?? '-'),
            'description'   => esc_html($bill_details['desc']['detail'][0] ?? 'Tidak ada deskripsi'),
            'bill_amount'   => wppob_format_rp($bill_amount),
            'admin_fee'     => wppob_format_rp($admin_fee + $profit_amount),
            'total_payment_raw' => $total_payment,
            'total_payment' => wppob_format_rp($total_payment),
            'ref_id'        => esc_html($bill_details['ref_id'])
        ];
        wp_send_json_success($data_to_send);
    } else {
        wp_send_json_error(['message' => $response['data']['message'] ?? 'Gagal mengecek tagihan.']);
    }
}
