...
class WPPOB_Products {
   ...
   public function calculate_sale_price($base_price) {
    // Cek status lisensi. Jika tidak valid, jangan tambahkan profit.
    $admin_instance = new WPPOB_Admin();
    if (!$admin_instance->is_license_valid()) {
        return ceil((float)$base_price); // Kembalikan harga dasar saja
    }

    $base_price = (float) $base_price;
    $profit_type = get_option('wppob_profit_type', 'fixed');
    $profit_amount = (float) get_option('wppob_profit_amount', 0); // Default profit 0
